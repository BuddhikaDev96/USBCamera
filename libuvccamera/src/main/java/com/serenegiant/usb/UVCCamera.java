...
public class UVCCamera {
    ...
    protected int mAutoWhiteBlancMin, mAutoWhiteBlanceMax, mAutoWhiteBlanceDef;
    ...
    public int mBrightnessMin, mBrightnessMax, mBrightnessDef;
    public int mContrastMin, mContrastMax, mContrastDef;
    public int mSharpnessMin, mSharpnessMax, mSharpnessDef;
    protected int mGainMin, mGainMax, mGainDef;
    protected int mGammaMin, mGammaMax, mGammaDef;
    public int mSaturationMin, mSaturationMax, mSaturationDef;
...
//	}
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat, int fpsCount) {
		setPreviewSize(width, height, fpsCount, fpsCount, frameFormat, mCurrentBandwidthFactor);
	}
...
//================================================================================
	public synchronized void setAutoWhiteBlance(final boolean autoWhiteBlance) {
    	if (mNativePtr != 0) {
    		nativeSetAutoWhiteBlance(mNativePtr, false);
    	}
    }
...
//================================================================================
	...
	public synchronized void setWhiteBlance(final int whiteBlance) {
    	if (mNativePtr != 0) {
			...
			System.out.println("max="+mWhiteBlanceMax);
			System.out.println("min="+mWhiteBlanceMin);
 		   if (range > 0)
...
//================================================================================
	...
	public synchronized void setContrast(final int contrast) {
    	if (mNativePtr != 0) {
			...
			System.out.println("setContrast range="+range);
	    	if (range > 0)
	...
	public synchronized void setSharpness(final int sharpness) {
    	if (mNativePtr != 0) {
 		  final float range = Math.abs(mSharpnessMax - mSharpnessMin);
			...
			System.out.println("range="+range);
 		   if (range > 0)
			   System.out.println("shapness="+(sharpness / 100.f * range) +"");
 			   nativeSetSharpness(mNativePtr, (int)(sharpness / 100.f * range) );
    	}
    }
    	...
    	if (mNativePtr != 0) {
    		if ((mControlSupports == 0) || (mProcSupports == 0)) {
    	    	...
    	    	if ((mControlSupports != 0) && (mProcSupports != 0)) {
	    	    	...
	    	    	nativeUpdateWhiteBlanceLimit(mNativePtr);
	    	    	nativeUpdateFocusLimit(mNativePtr);
	    	    	nativeUpdateAutoWhiteBlanceLimit(mNativePtr);
	    	    	nativeUpdateAutoWhiteBlanceCompoLimit(mNativePtr);
    	    	}
    	    	if (DEBUG) {
